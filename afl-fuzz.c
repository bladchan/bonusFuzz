...
EXP_ST void setup_shm(void) {
  ...
  shm_id = shmget(IPC_PRIVATE, MAP_SIZE + sizeof(struct edge_map), IPC_CREAT | IPC_EXCL | 0600);
...
EXP_ST void init_forkserver_bonus(char** argv) {
    ...
    if (bonus_fsrv_pid < 0) PFATAL("fork() failed");
    ...
    bonus_fsrv_ctl_fd = ctl_pipe[1];
    bonus_fsrv_st_fd = st_pipe[0];
    ...
    rlen = read(bonus_fsrv_st_fd, &status, 4);
    ...
    if (waitpid(bonus_fsrv_st_fd, &status, 0) <= 0)
        PFATAL("waitpid() failed");
...
static u8 run_target_bonus(char** argv, u32 timeout) {
    ...
    memset(trace_bits, 0, MAP_SIZE + sizeof(struct edge_map));
    MEM_BARRIER();
    ...
    if (dumb_mode == 1 || no_forkserver) {
        ...
        if ((res = write(bonus_fsrv_ctl_fd, &prev_timed_out, 4)) != 4) {
        ...
        if ((res = read(bonus_fsrv_st_fd, &child_pid, 4)) != 4) {
    ...
    if (dumb_mode == 1 || no_forkserver) {
        ...
        if ((res = read(bonus_fsrv_st_fd, &status, 4)) != 4) {
...
abort_calibration:
  ...
  if (!bonus_mode) return fault;

  /* Here we do some analysis... */

  write_to_testcase(use_mem, q->len);
  run_target_bonus(argv, use_tmout * 3);

  struct edge_map* map = (struct edge_map* )(u8*)(trace_bits + MAP_SIZE);
  struct edge_node* p = NULL;
      ...
      for (int j = 0; j < map->headers[i].size2; j++) {
          ...
          if (!is_traced) {
              ...
              if (!q->untouch) {
                  ...
                  q->untouch = ck_alloc(sizeof(struct edge_node));
                  p = q->untouch;

              }
              else {

                  p->next = ck_alloc(sizeof(struct edge_node));
                  p = p->next;

              }

              p->pre_bb_id = map->untouch[i][j].pre_bb_id;
              p->edge_id = i;
              p->bonus = map->untouch[i][j].bonus;
              p->next = NULL;
  ...
  struct queue_entry* tmp_q = queue;

  while (tmp_q) {

      if (tmp_q == q) { break; }

      struct edge_node* prev = ck_alloc(sizeof(struct edge_node)), *t = prev;
      ...
      while (p) {
          ...
          if (map->headers[p->edge_id].size1) {

              for (int i = 0; i < map->headers[p->edge_id].size1; i++) {

                  if (map->trace[p->edge_id][i].pre_bb_id == p->pre_bb_id) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:b:t:T:dnCB:S:M:x:QVr:")) > 0)
    ...
    switch (opt) {
      ...
      case 'r': /* bonus mode */
        ...
        if (bonus_mode) FATAL("Multiple -r options not supported");
        bonus_mode = 1;
